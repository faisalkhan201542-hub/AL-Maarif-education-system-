...
const getSubjectColor = (subject) => {
  if (!subject) return 'bg-transparent text-gray-800';
...
export default function TimetablePage() {
  ...
  const renderDailyMatrix = () => {
    ...
    return (
      <div className="w-full overflow-x-auto border border-[#e5e7eb] rounded-xl shadow-sm bg-white">
            ...
            {periodsArray.map((p) => {
              ...
              return (
                <tr key={p} className="hover:bg-gray-50/50 transition-colors">
                  <td className="p-3 bg-[#f8f9fa] border-b border-r border-gray-200 font-bold text-gray-700 text-center text-sm sticky left-0 z-10 whitespace-nowrap shadow-[1px_0_0_0_#e5e7eb]">
                    {timeSlot.start}<br/><span className="text-gray-400 font-normal">to</span><br/>{timeSlot.end}
                  </td>
                  ...
                  {CLASSES.map(cls => {
                    ...
                    return (
                      <td key={cls} className="p-2 border-b border-r border-gray-100 align-middle">
                        {period ? (
                          <div className={`w-full h-full min-h-[75px] rounded p-2 flex flex-col justify-center items-center text-center transition-all relative group/cell ${getSubjectColor(period.subject)}`}>
                            ...
                            <div className="absolute inset-0 bg-white/95 backdrop-blur-[1px] opacity-0 group-hover/cell:opacity-100 transition-opacity flex items-center justify-center gap-3 rounded border border-gray-200">
                          ...
                          <div className="w-full h-full min-h-[75px] flex items-center justify-center relative group/cell rounded hover:bg-gray-50 transition-colors">
                            <span className="text-gray-300 text-sm font-medium">-</span>
                            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/cell:opacity-100 transition-opacity">
                               <button onClick={() => openForm(null, cls, p)} className="p-1.5 bg-gray-100 rounded-full text-gray-500 hover:bg-blue-500 hover:text-white transition-all shadow-sm">
                                 <Plus size={16} />
                               </button>
                            </div>
                          </div>
    ...
    <div className="space-y-6 max-w-full pb-10">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div className="text-center mb-6">
          <p className="text-xs font-bold tracking-[0.2em] text-[#b28e46] uppercase mb-2">DAILY SCHEDULE</p>
          <h1 className="text-3xl sm:text-4xl font-serif font-bold text-[#2c405a] mb-2">Class Timetable</h1>
          <p className="text-sm text-gray-500">Play Group through Class 10th · 8:00 AM – 2:00 PM</p>
        </div>
      ...
      <div className="flex justify-center overflow-x-auto pb-2">
        <div className="inline-flex bg-white shadow-sm border border-gray-100 p-1.5 rounded-xl whitespace-nowrap">
          {DAYS.map(day => (
            <button
              key={day}
      ...
      {showForm && (
        <div className="bg-white shadow-xl rounded-xl p-6 border border-[#2c405a]/10 relative">
          <button 
          ...
          <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-4 gap-5">
            <div>
              ...
              <select className="w-full px-4 py-2 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:ring-2 focus:ring-[#2c405a] focus:border-transparent transition-all outline-none" value={form.day} onChange={e => setForm({...form, day: e.target.value})}>
                {DAYS.map(d => <option key={d} value={d}>{d}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Class</label>
              <select className="w-full px-4 py-2 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:ring-2 focus:ring-[#2c405a] focus:border-transparent transition-all outline-none" value={form.class} onChange={e => setForm({...form, class: e.target.value})}>
                {CLASSES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Period Number</label>
              <input type="number" min="1" required className="w-full px-4 py-2 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:ring-2 focus:ring-[#2c405a] focus:border-transparent transition-all outline-none" value={form.periodNumber} onChange={e => handlePeriodChange(Number(e.target.value))} />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Subject</label>
              <input required className="w-full px-4 py-2 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:ring-2 focus:ring-[#2c405a] focus:border-transparent transition-all outline-none" placeholder="e.g. Maths" value={form.subject} onChange={e => setForm({...form, subject: e.target.value})} />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Teacher</label>
              <select 
                required
                className="w-full px-4 py-2 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:ring-2 focus:ring-[#2c405a] focus:border-transparent transition-all outline-none" 
                value={form.teacher} 
            ...
            <div>
              ...
              <input required className="w-full px-4 py-2 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:ring-2 focus:ring-[#2c405a] focus:border-transparent transition-all outline-none" placeholder="08:00 AM" value={form.startTime} onChange={e => setForm({...form, startTime: e.target.value})} />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">End Time</label>
              <input required className="w-full px-4 py-2 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:ring-2 focus:ring-[#2c405a] focus:border-transparent transition-all outline-none" placeholder="08:45 AM" value={form.endTime} onChange={e => setForm({...form, endTime: e.target.value})} />
            </div>
            <div className="md:col-span-4 flex justify-end gap-3 mt-4 pt-4 border-t border-gray-100">
              <button type="button" onClick={() => setShowForm(false)} className="px-5 py-2.5 text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg font-medium transition-colors">
                Cancel
              </button>
      ...
      {loading ? (
        <div className="flex justify-center p-12">
          <p className="text-gray-500 animate-pulse font-medium text-lg">Loading timetable...</p>
        </div>
      ) : (
        renderDailyMatrix()
      )}
